...
window.addRepair = function() {
    ...
    function saveRepair(imageData) {
        ...
        const newRepair = {
            id: Date.now(),
            user: currentUser.username,      // ✅ ใช้ name จริง
            email: currentUser.email,    // ✅ email จริง
            item: item,
            detail: detail,
            image: imageData,
            status: "รอดำเนินการ",
        ...
        emailjs.send("service_2danuhi", "template_i87v8e6", {
            user_name: newRepair.user,
            item: newRepair.item,
            detail: newRepair.detail,
            date: newRepair.date,
            time: newRepair.time,
            status: newRepair.status,
            image: newRepair.image   // 🔥 เพิ่มบรรทัดนี้
        })
        .then(function(response) {
            console.log("แจ้งเจ้าหน้าที่สำเร็จ", response.status);
        })
        .catch(function(error) {
            console.log("ส่งหาเจ้าหน้าที่ไม่สำเร็จ", error);
        });
    ...
    repairs.reverse().forEach(r => {
        ...
        li.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: baseline;">
                ...
                <span style="font-size: 0.8em; color: #888;">${r.date || ''} ${r.time ? '⏰ '+r.time : ''}</span>
            </div>
...
window.updateRepairStatus = function(id, newStatus) {
    ...
    // 🔥 ส่งอีเมลแจ้งผู้ใช้
    emailjs.send("service_2danuhi", "template_lqpoqiw", {
        user_name: repair.user,
        item: repair.item,
    ...
    .then(function(response) {
        console.log("แจ้งผู้ใช้สำเร็จ", response.status);
    })
    .catch(function(error) {
        console.log("ส่งหาผู้ใช้ไม่สำเร็จ", error);
    });

    alert("อัปเดตสถานะและแจ้งผู้ใช้แล้ว");
    renderRepairs();
};
    ...
    if(historyList){
        ...
        myRepairs.forEach(r => {
            ...
            li.innerHTML = `
                ...
                <p><strong>วันที่แจ้ง:</strong> ${r.date} ${r.time ? '⏰ '+r.time : ''}</p>
